// Example poker game from Appendix B of Absolute Beginner's Guide
// File AppendixBpoker.c

/*	This program plays draw poker. Users can play as often as they want,
	betting between 1 and 5. They are dealt 5 cards and then get to choose
	which cards to keep, and which cards to replace. The new hand is then reviewed and the user's 
	payout is set based on the value of the hand. The user's new bankroll is displayed as
	they are given the option to continue. 	*/
	...
	// Two constants for determining wether hands are flushes or straights
	...
	main()
	{
		...
		// only needs to be displayed once, while eveything else in main will run
	...
	// Print a quick greeting as well as tell the users the value of different winnig hands
	...
	// Function that changes the rank integer value to a char repping the rank
	...
	int getBet()
	{
		...
		{
			printf("How much do you want to bet? (Enter a Number 1 to 5, or 0 to quit) : ");
			scanf(" %d", &bet);
	...
	int analyzeHand(int ranksinHand[], int suitsinHand[])
	{
		...
		// This function looks through each of the five cards in the first hand and asks the user
		// if they want to keep the card. If they say no, they get a replacement card.
			...
			for (i=0; i < 5; i++){
				...
				if (toupper(ans) == 'Y')
				{
					...
					do {
						...
						// First chck your new card against the 5 original cards to avoid duplication
						for (j=0; j < 5; j++){
